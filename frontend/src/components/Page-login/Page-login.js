...
function PageLogin() {
	...
	const initPartidaInterval = (id) => {
		intervalPartida = setInterval(() => {
			carregarDadosPartida(id)
		}, 1000)
	}
	...
	const sairPartida = () => {
		...
		fetch(
			`http://127.0.0.1:${port}/partida/sair/${partida.id}/user/${participante.id}`,
			{
				method: "POST",
			}
		)
		...
		<div className="connected">
			...
			{view === "ROOMS" ? (
				<Rooms
					...
					entrarPartida={(id) => entrarPartida(id)}
				/>
			) : (
				<Container
					cards={cards}
					...
					rodada={{}}
					onSelectCard={(text) => onSelectCard(text)}
				/>
			)}
			<Reconectar
				show={showModalReconect}
				recuse={onRecuseReconect}
				accept={onReconect}
			/>
			<Winner
				show={openWinners}
				confirm={confirmFinalPartida}
